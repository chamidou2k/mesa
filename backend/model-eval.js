...
function model_eval_old(model_topo, model_para, context_para, hw_spec) {
...
/*
  Refinement:
  1. add fine-grained control for quantization bit width
  2. re-structure the code to support statistical analysis based on different layer definitions
  3. pipeline and expert parallelism supportt
*/
function model_eval(model_topo, model_para, context_para, hw_spec) {
    ...
    const vocab_size = model_para.vocab_size;
    const total_gpu = context_para.tp * context_para.pp * context_para.dp;
    let n_local_experts = 1, n_local_shared_experts = 1, n_local_experts_activate_ratio = 1.0; // local experts number in each devicee
    const pp_latency = hw_spec.scaleout_delay;
    if (context_para.ep > 1 ) {
	if (model_para.n_routed_experts !== undefined)  
	    n_local_experts = model_para.n_routed_experts / context_para.ep;
	if (model_para.num_experts_per_tok !== undefined)  
	    n_local_experts_activate_ratio = model_para.num_experts_per_tok / model_para.n_routed_experts;
        if (model_para.n_shared_experts !== undefined) 
	    n_local_shared_experts = Math.max(1, Math.floor(model_para.n_shared_experts / context_para.ep));
    }
    const total_comp = {
        prefill: { flops: 0, memory: 0, latency: 0 },
    ...
    // total_combo indicates the total computation for each phase
    const total_combo = {
	prefill: {},
	regression: {}
    };
	...
	if (comboDevs.has("ATTN")) {
	    const attn = comboDevs.get("ATTN");
	    fillComp(attn_device, attn);
	    console.log(`attn_device xxxx: ${JSON.stringify(attn_device)}`);
	}
	if (comboDevs.has("MoE")) {
		const moe = comboDevs.get("MoE");
		fillComp(moe_device, moe);
	...
	/*
	  scale at device level:
	  1. scale the routed and shared device by the number of local experts
	  2. total, weight, acts memory is scaled by the number of local experts (active experts + non-active experts)
	  3. for routed experts, flops and access memory are scaled by the number of activate experts
	 */
	const expert_device = {
	    mem: {
		total: routed_device.mem.total + shared_device.mem.total,
		weight: routed_device.mem.weight + shared_device.mem.weight,
		kv: routed_device.mem.kv + shared_device.mem.kv,
		access: routed_device.mem.access * n_local_experts_activate_ratio  + shared_device.mem.access,
		acts: routed_device.mem.acts + shared_device.mem.acts
	    },
	    flops: routed_device.flops * n_local_experts_activate_ratio + shared_device.flops,
	    latency: routed_device.latency * n_local_experts_activate_ratio + shared_device.latency
	};
	mergeComp(moe_device, expert_device);
	...
	fillComp(moe_per_dev, moe_device);
	scaleComp(moe_per_dev, Math.floor(moe_num_layers / context_para.pp)); //scale by the moe number layer of layerer
	
	fillComp(densemlp_per_dev, densemlp_device);
	scaleComp(densemlp_per_dev, num_layers - moe_num_layers); // densmlp only scale to single devicee
	/*
	  condition 1: moe
	      ffn = ffn + moe + densemlp
	  condition 2: non-moe
	      ffn = ffn 
	  */
	fillComp(ffn_per_dev, ffn_device);
	scaleComp(ffn_per_dev, Math.floor(num_layers / context_para.pp)); //scale by the number of layers
	mergeComp(ffn_per_dev, moe_per_dev); //moe_per_dev will be 0 if no moe
	mergeComp(ffn_per_dev, densemlp_per_dev); // densemlp_per_dev will be 0 if no densemlp
	
	fillComp(attn_per_dev, attn_device);
	scaleComp(attn_per_dev, Math.floor(num_layers / context_para.pp)); //scale by the number of layers;
	mergeComp(total_per_dev, attn_per_dev);
	mergeComp(total_per_dev, ffn_per_dev);
	mergeComp(total_per_dev, input_device);
	mergeComp(total_per_dev, output_device);
	fillComp(total_comp_final, total_per_dev);
	scaleComp(total_comp_final, context_para.tp); // scale by the number of tp
	total_comp_final.latency /= context_para.tp; //latency won't be scaled by tp
	scaleComp(total_comp_final, context_para.pp); // scale by the number of pp
	total_comp_final.latency += (pp_latency * context_para.pp); //latency won't be scaled by pp
	scaleComp(total_comp_final, context_para.dp); // scale by the number of dp
	total_comp_final.latency /= context_para.dp; //latency won't be scaled by dp
	//commented out for debug
	console.log(`ffn_per_dev: ${JSON.stringify(ffn_per_dev)}`);
	console.log(`attn_per_dev: ${JSON.stringify(attn_per_dev)}`);
	console.log(`moe_per_dev: ${JSON.stringify(moe_per_dev)}`);
	console.log(`densemlp_per_dev: ${JSON.stringify(densemlp_per_dev)}`);
	console.log(`input_per_dev: ${JSON.stringify(input_per_dev)}`);
	console.log(`output_per_dev: ${JSON.stringify(output_per_dev)}`);
	console.log(`total_per_dev: ${JSON.stringify(total_per_dev)}`);
	console.log(`total_comp_final: ${JSON.stringify(total_comp_final)}`);
	console.log(`${hw_spec.mem_size}, ${(nonlayer_total_memory + layer_total_memory * num_layers + acts_mem_peak)}`);
...
// extend formula_eval to support multiple parameters
function formula_eval(formula, para) {
    let matchResult = formula.match(/\[(.*?)\]/); 
    if (matchResult) {
        let paramsStr = matchResult[1];
	console.log(paramsStr);
        let paramNames = paramsStr.split(/\s*,\s*/);
	console.log(paramNames);
        return paramNames.map(name => math.evaluate(name, para));
    } else {
        return [0, 0];
    }	
}
//temporary backward compatibility
function formula_eval_old(formula, para) {
    let matchResult = formula.match(/\[(.*?)\]/); 
    if (matchResult) {
	let paramsStr = matchResult[1]; 
	console.log(paramsStr);
...
function calculateNodeMetrics(node, ctx, params, model_para, phase) {
    ...
    const quant_method = model_para.quantization_config?.quant_method || 'none';
    console.log(`quantization_config: ${JSON.stringify(model_para.quantization_config)} ${quant_method}`);
    
    let gemm_quant = false;
    try {
	let ret;
	console.log(`Calculating metrics for node: ${node.id}, op_type: ${op_type}, pre_shape: ${JSON.stringify(pre_shape)}`);
        switch (node.id) {
	case 'input_embed':
    ...
    calculateLatencyInfo(node_comp, dev_comp, ctx, gemm_quant, quant_method);
    calculateBoundInfo(node_comp);
    calculateBoundInfo(dev_comp);
    console.log(`node_comp: ${JSON.stringify(node_comp)}`);
    console.log(`dev_comp: ${JSON.stringify(dev_comp)}`);
    ...
    function calculateLatencyInfo(dst_node, dst_dev, ctx, gemm_quant, quant_method) {
	let effective_flops = hw_spec.flops;
	if (gemm_quant && quant_method && quant_method !== 'none' && hw_spec[`${quant_method}_flops`]) {
	    effective_flops = hw_spec[`${quant_method}_flops`];
        } else {
	    effective_flops = hw_spec.flops;
	}
	const __calc = (target, tp, ef, qu) => {
            const { mem, flops, comm_para } = target;
            const latency = {
		mem: (mem.access / (hw_spec.mem_bw * hw_spec.mem_bw_util * tp)) * 1e6,
		comp: (flops / (ef * tp)) * 1e6,
	...
	const devLatency = __calc(dst_dev, 1, effective_flops, gemm_quant ? quant_method : null);
	const nodeLatency = __calc(dst_node, ctx.tp, effective_flops, gemm_quant ? quant_method : null);
